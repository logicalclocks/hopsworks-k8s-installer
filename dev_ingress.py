...
def setup_ingress(namespace):
    ...
    success, output, error = run_command("kubectl get pods -n ingress-nginx -l app.kubernetes.io/name=ingress-nginx", verbose=False)
    if not success or "No resources found" in output:
        print_colored("NGINX Ingress Controller is not installed. Installing it now...", "yellow")
        ...
        success, output, error = run_command(install_cmd)
        if not success:
            print_colored("Failed to install NGINX Ingress Controller.", "red")
            print_colored(f"Error: {error}", "red")
            sys.exit(1)
        print_colored("NGINX Ingress Controller installed successfully.", "green")
    else:
        print_colored("NGINX Ingress Controller is already installed.", "green")
    
    # Wait for the Ingress Controller to be ready
    print_colored("Waiting for NGINX Ingress Controller to be ready...", "blue")
    success = wait_for_pods_ready("ingress-nginx", "app.kubernetes.io/name=ingress-nginx", timeout=300)
    if not success:
        print_colored("Timed out waiting for NGINX Ingress Controller to be ready.", "red")
        sys.exit(1)
    ...
    for _ in range(30):  # Try for 5 minutes
        success, output, error = run_command("kubectl get service ingress-nginx-controller -n ingress-nginx -o jsonpath='{.status.loadBalancer.ingress[0].ip}'", verbose=False)
        if success and output.strip():
            ingress_address = output.strip()
            break
        
        success, output, error = run_command("kubectl get service ingress-nginx-controller -n ingress-nginx -o jsonpath='{.status.loadBalancer.ingress[0].hostname}'", verbose=False)
        if success and output.strip():
            ingress_address = output.strip()
            break
...
"""
    ...
    print_colored("\nPlease add the following entry to your /etc/hosts file:", "blue")
    ...
    # Offer to automatically update /etc/hosts
    update_hosts = input("Would you like the script to automatically update your /etc/hosts file? (yes/no): ").strip().lower() == 'yes'
    if update_hosts:
        try:
            ...
            print_colored(f"Failed to update /etc/hosts file: {str(e)}", "red")
...
def wait_for_pods_ready(namespace, label_selector, timeout=300):
    ...
    while time.time() - start_time < timeout:
        success, output, error = run_command(f"kubectl get pods -n {namespace} -l {label_selector} -o jsonpath='{{.items[*].status.phase}}'", verbose=False)
        if success and all(status == "Running" for status in output.split()):
            return True
        time.sleep(5)
    return False

def get_ingress_address(namespace, service_name):
    for _ in range(30):  # Try for 5 minutes
        success, output, error = run_command(f"kubectl get service {service_name} -n {namespace} -o jsonpath='{{.status.loadBalancer.ingress[0].ip}}'", verbose=False)
        if success and output.strip():
            return output.strip()
        
        success, output, error = run_command(f"kubectl get service {service_name} -n {namespace} -o jsonpath='{{.status.loadBalancer.ingress[0].hostname}}'", verbose=False)
        if success and output.strip():
            return output.strip()
...
def delete_namespace(namespace):
    ...
    custom_resources = [
        "hopsworkscert.certs.hopsworks.ai",
        "remoteshuffleservice.uniffle.apache.org"
    ]
    
    for resource in custom_resources:
        print_colored(f"Removing finalizers from {resource}...", "cyan")
        run_command(f"kubectl get {resource} -n {namespace} -o name | xargs -r -I{{}} kubectl patch {{}} -n {namespace} -p '{{\"metadata\":{{\"finalizers\":[]}}}}' --type=merge")
    
    # Remove finalizers from the namespace
    print_colored("Removing finalizers from the namespace...", "cyan")
    run_command(f"kubectl get namespace {namespace} -o json | tr -d \"\\n\" | sed \"s/\\\"finalizers\\\": \\[[^]]*\\]/\\\"finalizers\\\": []/\" | kubectl replace --raw /api/v1/namespaces/{namespace}/finalize -f -")
    
    # Delete the namespace forcefully
    print_colored(f"Forcefully deleting namespace {namespace}...", "cyan")
    success, _, error = run_command(f"kubectl delete namespace {namespace} --force --grace-period=0")
    
    if success:
        print_colored(f"Namespace '{namespace}' deleted successfully.", "green")
        return True
    else:
        print_colored(f"Failed to delete namespace '{namespace}'. Error: {error}", "red")
        return False
        
def cleanup_custom_resources(namespace):
    print_colored(f"Cleaning up custom resources in namespace '{namespace}'...", "blue")
    
    # List of custom resource types to clean up
    custom_resources = [
        "hopsworkscert.certs.hopsworks.ai",
        "remoteshuffleservice.uniffle.apache.org"
    ]
    
    for resource_type in custom_resources:
        print_colored(f"Deleting {resource_type} resources...", "cyan")
        success, output, error = run_command(f"kubectl get {resource_type} -n {namespace} -o name | xargs -r kubectl delete -n {namespace}")
        if not success:
            print_colored(f"Error deleting {resource_type} resources: {error}", "yellow")
        else:
            print_colored(f"Successfully deleted {resource_type} resources.", "green")
    
    print_colored("Custom resource cleanup completed.", "green")
    ...
    if namespace_exists(namespace):
        ...
        if delete.lower() == "yes":
            if delete_namespace(namespace):
                print_colored(f"Namespace '{namespace}' and its resources have been deleted.", "green")
            else:
                print_colored(f"Failed to completely delete namespace '{namespace}'. Manual intervention may be required.", "red")
                return
        else:
